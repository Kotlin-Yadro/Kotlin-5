...
class NaturalList(n: Int) : List<Int> {
    ...
    override fun subList(fromIndex: Int, toIndex: Int): List<Int> {
        ...
        var allowedFromIndex = fromIndex
        var allowedToIndex = toIndex
        if (allowedFromIndex < 0) {
            allowedFromIndex = 0
        }
        if (allowedToIndex > size){
            allowedToIndex = size
        }
        if (allowedFromIndex < allowedToIndex) {
            for (i in allowedFromIndex..< allowedToIndex) {
